...
workflow-driven failover solution with no need to modify or DB-checkpoint
...
Choose convenient [sync points](#sync-points) in each workflow.
 - The more sync points, the less re-running of tasks after failover.
    - (Start with a few sync points, automate, then add more as desired.)
 - At sync points, explicitly transfer associated restart data to the other platform.
    ...
    - (This positioning could be done during transfer, or later during failover.)
